...
MAX_DIST_FIRST_FRAME = 15
MANUAL_LEFT_MASK_SHIFT = 100   # pixels to move the left bound to the right
MANUAL_RIGHT_MASK_SHIFT = 0   # usually 0; positive would move bound left
...
def remove_short_edges_guarded(G, max_edge_len, max_iter=50):
    """
    Remove edges shorter than max_edge_len *only if* they are redundant
    (i.e. their endpoints remain connected after removing the edge).

    This is used for the first frame, after triangle + deg1 cleanup,
    to kill very small cycles without opening the main cycles.

    Parameters
    ----------
    G : networkx.Graph
        Graph with node attribute 'o' = (row, col) in pixels.
    max_edge_len : float
        Maximum allowed length for an edge (in pixels).
    max_iter : int
    ...
    for it in range(max_iter):
        ...
        # work on a snapshot of edges for this iteration
        for u, v, data in list(G.edges(data=True)):
            ...
            # candidate short edge; test if it's redundant
            attrs = data.copy()
            G.remove_edge(u, v)

            # if u and v are still connected, the edge was part of a cycle
            # but not essential (no "open cycle"), so we keep it removed
            if nx.has_path(G, u, v):
                removed_any = True
                # optional: you could contract degree-2 nodes here if desired
            else:
                # edge is essential; restoring it
    ...
    print(f"[short-edges] finished guarded cleanup; edges = {G.number_of_edges()}")
...
df = pd.read_excel(file_path_zwick, sheet_name='Ex4')
...
def row_analysis_fct_frame_0(horizontal_image,
    ...
    # --- find the valley between the two strongest peaks (flute centreline) ---
    i_min = min(peak_coord_max_2[:, 0])
    i_max = max(peak_coord_max_2[:, 0])
    ...
    # 1) primary estimates based on liner centres and thickness
    board_upper_limit_estimation = (
        upper_liner_center_pos - liners_thickness_estimation // 2
    )
    board_lower_limit_estimation = (
        lower_liner_center_pos + liners_thickness_estimation // 2
    )
    
    # 2) OPTIONAL: small adjustment of bottom using the valley,
    #    but only if valley is close to the liner-based bottom
    BOARD_VALLEY_OFFSET = -1      # tweak: -1, 0, +1
    MAX_VALLEY_SHIFT   = 10       # max allowed correction in pixels
    
    candidate_bottom = valley_idx + BOARD_VALLEY_OFFSET
    if abs(candidate_bottom - board_lower_limit_estimation) <= MAX_VALLEY_SHIFT:
        board_lower_limit_estimation = candidate_bottom
    
    if show_plots:
        plt.axvline(x=valley_idx, color="purple", linestyle="-.",
                    label="Valley (flute centre)")
        plt.axhline(y=board_upper_limit_estimation, color="green",
                    linestyle="--", label="Board upper")
        plt.axhline(y=board_lower_limit_estimation, color="green",
                    linestyle="--", label="Board lower")
        plt.legend()
...
if new_image_analysis: 
    ...
    # rotate the first frame (or not, if ROTATION_ANGLE is None)
    if ROTATION_ANGLE is None:
        # no rotation: just use original grayscale image
        horizontal_image = gray_image.copy().astype(np.uint8)
    else:
    ...
    BOARD_BOTTOM_ROW_FIXED = board_lower_limit_estimation
   ...
   ############## Skeletonize and build a graph from the filtered frame 0
    ...
    remove_short_edges_guarded(
        graph_0,
        max_edge_len=MAX_DIST_FIRST_FRAME,
        max_iter=30,
    )
    
    print("[DEBUG] after short-edge cleanup:",
          graph_0.number_of_nodes(), "nodes,",
          graph_0.number_of_edges(), "edges")
    
    # Final cleanup: remove interior degree-2 nodes again
    graph_0 = compress_degree2_paths(graph_0, border_x_tol=15)
    prune_degrees(graph_0, border_x_tol=15)
    
    print("[DEBUG] after final prune_degrees:",
          graph_0.number_of_nodes(), "nodes,",
          graph_0.number_of_edges(), "edges")
    ...
    nodes = graph_0.nodes()
    edges = graph_0.edges()
    print("\n===== FRAME 0: node degrees and neighbors =====")
    for n in sorted(graph_0.nodes()):
        deg = graph_0.degree(n)
        neighbors = sorted(graph_0.neighbors(n))
        print(f"Frame 0 - node {n:3d}: degree={deg:2d}, neighbors={neighbors}")
    print("===== END FRAME 0 =====\n")
    
    import math

    print("=== Edge lengths in graph_0 ===")
    for u, v in sorted(graph_0.edges()):
        r1, c1 = graph_0.nodes[u]["o"]   # (row, col)
        r2, c2 = graph_0.nodes[v]["o"]
        dy = int(r2) - int(r1)   # force Python ints, no wrap-around
        dx = int(c2) - int(c1)
        d = math.hypot(dx, dy)
        print(f"edge {u:3d}-{v:3d}: length = {d:7.3f} px")
    print("=== end ===")
    ...
    for frame_number in range(1, num_frames):
        ...
        # rotate the image (or not, if ROTATION_ANGLE is None)
        if ROTATION_ANGLE is None:
            current_horizontal_image = current_gray_img.copy().astype(np.uint8)
        else:
        ...
        # ----------------------------------------------------------
        # 1) Row analysis for this frame – use *_curr so we don't overwrite frame-0 globals
        (upper_liner_center_pos_curr, lower_liner_center_pos_curr,
         board_upper_limit_estimation_curr, board_lower_limit_estimation_curr,
         upper_liner_inner_limit_estimation_curr,
         lower_liner_inner_limit_estimation_curr,
         Flute_center_line_curr,
         dist_liners_center_line_curr,
        ...
        # OPTIONAL: if you want thickness relative to the fixed bottom, recompute it here:
        Board_thickness_estimation = abs(
            board_upper_limit_estimation_curr - BOARD_BOTTOM_ROW_FIXED
        ) + 1
        ...
        for pre_node, pre_row0, pre_col0 in nodes_coord_array:
            ...
            for c_node, c_row, c_col in nodes_coord_array_current:
                ...
                dy = int(c_row) - int(pre_row)
                dx = int(c_col) - int(pre_col)
                d = math.hypot(dx, dy)
        ...
        # ----------------------------------------------------------
        # First half of segments: i = 0, 4, 8, ...
        # ----------------------------------------------------------
        alpha_list = []
        count_edge = -1
        ...
        for i in range(0, max(nodes_2_dict.keys()), 4):
            ...
            if len(alpha_list_master[frame_number]) < 22:
                ...
                # only compute if both nodes exist in df_node_pos
                if i in df_node_pos.index and (i + 1) in df_node_pos.index:
                    ...
                    # print resulting angle for debugging
                    print(
                        f"[ANGLE] frame {frame_number:3d}, segment ({i:3d},{i+1:3d}), "
                        f"alpha = {alpha:7.3f} deg"
                    )

                    # keep the crossing detection logic, but guard the index
                    if frame_number > 1 and count_edge < len(alpha_list_master[frame_number-1]):
                        if (alpha_list_master[frame_number-1][count_edge] < 0
                                and not alpha_list_master_2[count_edge]):
        ...
        # ----------------------------------------------------------
        # Second half of segments: i = 2, 6, 10, ...
        # ----------------------------------------------------------
        alpha_list = []
        ...
        for i in range(2, max(nodes_2_dict.keys()), 4):
            ...
            if len(alpha_list_master[frame_number]) < 22:
                ...
                if i in df_node_pos.index and (i + 1) in df_node_pos.index:
                    ...
                    # second-half crossing detection (note the > 0 check)
                    if frame_number > 1 and count_edge < len(alpha_list_master[frame_number-1]):
                        if (alpha_list_master[frame_number-1][count_edge] > 0
                                and not alpha_list_master_2[count_edge]):
                            alpha_list_master_2[count_edge] = not alpha_list_master_2[count_edge]
...
for i in range(len(frames_to_plot)):
    current_img_path = os.path.join(
        destination_folder_graphs,
        f"Graph_Frame_{frames_to_plot[i]:04d}.png"   # <-- use .png
    )
    current_gray_img = cv2.imread(current_img_path)
    if current_gray_img is None:
        raise FileNotFoundError(f"Could not read image: {current_img_path}")

    current_rgb_img = cv2.cvtColor(current_gray_img, cv2.COLOR_BGR2RGB)
    axs_master[i].imshow(current_rgb_img)
    axs_master[i].axis('off')
...
def graph_data (graph):
    ...
    nodes_coord_array_upper_liner = nodes_coord_array_new_ref[
        nodes_coord_array_new_ref[:, 2] > mean_y
    ].astype(int)
    # nodes with y-positions under / equal the mean value are lower liner nodes
    nodes_coord_array_lower_liner = nodes_coord_array_new_ref[
        nodes_coord_array_new_ref[:, 2] <= mean_y
    ].astype(int)
    
    # ---------- ROBUST THICKNESS ESTIMATION ----------
    if (
        nodes_coord_array_upper_liner.size == 0
        or nodes_coord_array_lower_liner.size == 0
    ):
        # Fallback: use global min/max of all node y's
        y_vals = nodes_coord_array_new_ref[:, 2]
        min_y = y_vals.min()
        max_y = y_vals.max()
        thickness_estimation = round(max_y - min_y)
    
        # no reliable split into upper/lower in this frame
        node_pos_dict_upper_liner = {}
        node_pos_dict_lower_liner = {}
    
    else:
        # Normal case: both upper and lower sets non-empty
...
def graph_data_mm(graph, pixel_mm ):
    ...
    if (nodes_coord_array_upper_liner.size == 0 or
            nodes_coord_array_lower_liner.size == 0):

        y_vals = nodes_coord_array_new_ref[:, 2]
        min_y = y_vals.min()
        max_y = y_vals.max()
        thickness_estimation = (max_y - min_y) * pixel_mm
...
if Node_clustering : 
    ...
    for sheet_name, data in excel_data.items():
        ...
        for _, row in data.iterrows():
            ...
            dx = int(new_col) - int(ref_col)        # x displacement in pixels
            dy = int(new_row) - int(ref_row)       # y displacement in pixels
