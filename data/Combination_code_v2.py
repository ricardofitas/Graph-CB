...
# sample name
#sample_name = 'Probe2_7'
sample_name = 'Probe1_22'
plt.ioff() 


# path of the Excel file
file_path_zwick = 'druckversuche_neuereinsatz einweisung (Fitas) 240524_1.xlsx'

# Read the sheet corresponding to a Specimen from the Excel file and save it 
# the measurement data in a data frame
df = pd.read_excel(file_path_zwick, sheet_name='Specimen 1.10')
...
def extract_frames(video_path, output_folder, extraction_rate):
    ...
    output_folder : string
        path of the folder where to store the frames.
    ...
    print(f'Frame rate (fps) of the original video: {fps_original}')
    
    
    if new_frame_extraction:
        
        # Read the first frame of the video
        success, image = video_capture.read()
    
        # initialisation of the counts
        frame_count = 0
        extracted_frame_count = 0
        
        print(f"Extracting frames from {video_path} into {output_folder}")
        ...
        while success:
            ...
            if frame_count % extraction_rate == 0:
                frame_filename = f"{frames_folder}/frame_{extracted_frame_count:04d}.jpg"
                cv2.imwrite(frame_filename, image)
                
                # increment the counter of extracted frames
                extracted_frame_count += 1
                
            # Read the next frame
            success, image = video_capture.read()
            
            # increment the counter of the total frames
            frame_count += 1
...
video_path = f"Sample_videos/{sample_name}.avi"
...
def display_masks_rgb_frame(horizontal_image, 
    ...
    # Define the top-left and bottom-right corners of the aria limited by the board limits
    top_left = (left_board_lim , board_upper_limit_estimation-1)
    bottom_right = (right_board_lim, board_lower_limit_estimation+1)
    # Define the color (BGR format) and thickness of the rectangle
...
def mask_filter_cleaning(img, 
    ...
    # # Define the white rectangular region of interest to clear the workspace
    mask_clear_workspace[board_upper_limit_estimation-1: board_lower_limit_estimation+1,  
...
if new_image_analysis: 
    ...
    Path_frame = f"{frames_folder}/frame_0000.jpg"
    
    # Read the grayscale image
    gray_image = cv2.imread(Path_frame, cv2.IMREAD_GRAYSCALE)
    ...
    # rough row analysis of the input image to define a symmetrical rough 
    # workspace around the board
    (image_upper_lim_rough, 
    image_lower_lim_rough) = rough_workspace_limits(gray_image)
    
    
    ### calculation of the rotation angle on the first frame if not already given
    ### The same rotation angle is kept form all the nextt frames 
    
    # manually give the rotation angle just to gain time while debagging.
    # Radon transform is quite time-consiuming.
    rotation_angle = -0.44
    ...
    try:
        ...
        # Variable does not exist, calculate it
        # cropp, center, reshaping and mask the image before sinogram analysis
        masked_square_image = reshape_and_mask(gray_image,
                                               image_upper_lim_rough, 
                                               image_lower_lim_rough,
                                               show_plots= True)
        # sinogram analysis and angle detection
        rotation_angle = sinogram_analysis(masked_square_image, show_plots= True)
        
        print(f"rotation angle calculated : {rotation_angle} degrees")
    
    else:
        # Variable exists, don't do anything
        print("rotation angle already exists. It will not be calculated again")
    
    
    # rotate the first frame
    horizontal_image = rotate(gray_image , rotation_angle, resize=False)
    # convert its values from 0..1 to 0..255
    horizontal_image = (horizontal_image * 255).astype(np.uint8)
    
    
    # create board limits to the right and left to make it so, as if we see 
    #   the hole sample, by setting some columns to zero
    #   this is done to the horizental image
    
    trim_size = 6
    horizontal_image = trim_L_R_limits(horizontal_image, trim_size)
    
    # row analysis of the horizontal frame 0 to : 
        # detect the upper and lower liners positions
        # set more precize workspace limits for the analysis coming after this
        # calculate estimations of geometric features
    
    # row analysis of the horizontal frame 0
    ...
    # Function to remove nodes with degree 1
    def remove_degree_one_nodes(G):
        # Step 1: Remove nodes with degree 1
        nodes_to_remove = [node for node, degree in dict(G.degree()).items() if degree == 1]
        while nodes_to_remove:
            G.remove_nodes_from(nodes_to_remove)
            nodes_to_remove = [node for node, degree in dict(G.degree()).items() if degree == 1]
    
        # Step 2: Identify the two most left-leaning and two most right-leaning nodes
        node_positions = {node: data['o'] for node, data in G.nodes(data=True)}
        sorted_nodes_by_x = sorted(node_positions.items(), key=lambda item: item[1][1])  # Sort by x-coordinate
        
        # Extract the two most left-leaning and right-leaning nodes
        left_most_nodes = {sorted_nodes_by_x[0][0], sorted_nodes_by_x[1][0]}
        right_most_nodes = {sorted_nodes_by_x[-1][0], sorted_nodes_by_x[-2][0]}
        boundary_nodes = left_most_nodes.union(right_most_nodes)
        
        # Step 3: Process nodes with degree 2 (excluding boundary nodes)
        nodes_to_remove = [node for node, degree in dict(G.degree()).items() 
                           if degree == 2 and node not in boundary_nodes]
        
        for node in nodes_to_remove:
            # Get neighbors of the node to be removed
            neighbors = list(G.neighbors(node))
            
            # Only connect if the node has exactly two neighbors
            if len(neighbors) == 2:
                n1, n2 = neighbors
                # Add an edge between the two neighbors if it doesn't exist
                if not G.has_edge(n1, n2):
                    G.add_edge(n1, n2)
            
            # Remove the node after connections are made
            G.remove_node(node)
    ...
    plt.figure(figsize=(fig_size , fig_size ))
    # draw image
    plt.title("Skeleton from frame number 0")
    plt.imshow(ske_skeletonize, cmap='gray')
    plt.axis('off')
    ...
    ##### rename the nodes column wise from left to right
    ...
    frames_to_plot = [1, 80, 160, 200]
    titles_list = ['Initial state', 'Middle of compression', 'Late compression', 'Post-compression']
    # Plot images on the subplots
    for i in range(len(frames_to_plot)):
        # Simulate 4 images with random data for demonstration
        current_img_path = os.path.join(frames_folder, f"frame_{frames_to_plot[i]:04d}.jpg")
        current_gray_img = cv2.imread(current_img_path, cv2.IMREAD_GRAYSCALE)   
        axs_master[i].imshow(current_gray_img, cmap='gray')
    ...
    # Iteration over all the frames starting by the second frame (frame_1.jpg)
    for frame_number in range( 1 , len(files)) :
        # define the path to the current frame
        current_img_path = os.path.join(frames_folder, f"frame_{frame_number:04d}.jpg")
        current_gray_img = cv2.imread(current_img_path, cv2.IMREAD_GRAYSCALE)
        
        # rotate the image
        current_horizontal_image = rotate(current_gray_img , rotation_angle, resize=False)
        # convert its values from 0..1 to 0..255
        current_horizontal_image = (current_horizontal_image * 255).astype(np.uint8)
    
        # board limits to the right and left as if we see the hole sample, 
        # by setting some columns to zeros
        current_horizontal_image = trim_L_R_limits(current_horizontal_image, trim_size)
        
        ## row analysis of the current horizontal frame: 
            # detect the upper and lower liners positions
            # calculate estimations of geometric features
        ...
        Flute_center_line,
        dist_liners_center_line,
        Board_thickness_estimation)  = row_analysis_fct_next_frame( current_horizontal_image,
                                                                    image_upper_lim,
                                                                    image_lower_lim,
                                                                    liners_thickness_estimation,
                                                                    show_plots= False)
        ...
        # Masks around the total board and cleaning of the environment, filtering
        #   correcting the liners missing
        current_filtered_image = mask_filter_cleaning(current_horizontal_image, 
                                                      show_plots = False,
        ...
        if Ske_thin_method :
            ...
            # build graph from skeleton
            current_graph = sknw.build_sknw(ske_skeletonize)
            
        else:
            # build graph from skeleton
            current_graph = sknw.build_sknw(ske_thin)
        
        
        graph_draw_fct (current_horizontal_image, current_graph,
                        title = "Skeleton graph from frame number " + str(frame_number),
                        node_labels=False,
                        zoom_in = False,
                        x_max = 500)
        
        # remqove nodes with degree 1
        remove_degree_one_nodes(current_graph)
        ...
        # # draw graph with nodes
        # graph_draw_fct (ske_skeletonize , current_graph,
        #                 title = f"Skeleton graph from frame number {frame_number}",
        #                 node_labels = False,
        #                 zoom_in = False,
        #                 x_max = 300)
        ...
        # initiate an array of zeros with number of rows equal to the number of nodes
        # and with 3 columns for [node_name , node_row_position , nodes_column_position]
        nodes_coord_array_current = np.zeros( (current_graph.number_of_nodes() + 10,3) ).astype(int)
        # initiate a counter of the node number to 0
        counter = 0
        ...
        for node in nodes:
            ...
            node_info_list_current = [node, nodes[node]['o'][0] , nodes[node]['o'][1]]
            nodes_coord_array_current[counter,:] = np.array(node_info_list_current)
            if nodes_coord_array_current[counter, 1] == 0 and nodes_coord_array_current[counter, 2] == 0:
                nodes_coord_array_current[counter, 0] = -1
            # increment the counter
            counter +=1
    
        for i in range(counter, nodes_coord_array_current.shape[0]):
            if nodes_coord_array_current[i, 1] == 0 and nodes_coord_array_current[i, 2] == 0:
                nodes_coord_array_current[i, 0] = -1
        
        # initialisation of an empty dictionary for the node tracking to associate the
        # old nodes to new nodes
        node_track_dict = {}
        
        associated = []
    
        # iteration over all the nodes of the old graph
        for pre_node, pre_row2, pre_col2  in nodes_coord_array:
            if not len(node_track_dict_dist[pre_node]) == 0:
                lista = node_track_dict_dist[pre_node]
                pre_row, pre_col = lista[-1][3:5]
            else:
                pre_row, pre_col = pre_row2, pre_col2
    
            # take the coord of one previous node
            p_pre = np.array([pre_row, pre_col])
            
            # initiate the minimum distance to a very hight value
            min_distance = 2000
            # initiate the new lable to any value
            new_node_label = 1000
            # iteration over all the nodes of the current graph
    
            for index, (c_node, c_row, c_col) in enumerate(nodes_coord_array_current):
                
                # take the coord of one new node
                p_curr = np.array([c_row, c_col])
                
                # Calculate the Euclidean distance
                distance = np.linalg.norm(p_pre - p_curr)
                
                if distance < min_distance and distance < 10 and not(c_node in associated ) :
                    
                    associated.append(c_node)
                    # take the new near point
                    new_node_label = c_node
                    new_row = c_row
                    new_col = c_col
                    # set the new min distance 
                    min_distance = distance
    
            if compression_phase == False and abs( c_row - pre_row) > 2:
                compression_phase = True
                compression_phase_start_frame = frame_number
            
            
            node_track_dict[new_node_label] = pre_node
            try:
                if min_distance < 10:
                    node_track_dict_dist[pre_node].append((frame_number, pre_row, pre_col, new_row, new_col, min_distance))
            except:
                print(1)
    
        
        # filter before relabeling to make current graph match the previous one
        
        
        
        ################################################################## step 1:
        # Find not associated nodes
        not_associated = set(associated) - set(graph_0.nodes())
        
        # print("Nodes not associated from G2:", not_associated)
        # print("Nodes unique to Graph 2:", unique_to_current_graph)
        
        # Adjust Graph 2 to match Graph 1
        
        # Remove nodes from Graph 2 that are not in Graph 1
        current_graph.remove_nodes_from(not_associated)
        ...
        ################################################################## step 2:
        # rename the nodes based on the dictionary created
        current_graph = nx.relabel_nodes(current_graph, node_track_dict)
        
        
        
        # adjastment after relabeling
        
        # Find unique nodes
        unique_to_graph_0 = set(graph_0.nodes()) - set(current_graph.nodes())
        unique_to_current_graph = set(current_graph.nodes()) - set(graph_0.nodes())
        
        # print("Nodes unique to Graph 1:", unique_to_graph_0)
        # print("Nodes unique to Graph 2:", unique_to_current_graph)
        ...
        ################################################################## step 3:
        # Remove nodes from Graph 2 that are not in Graph 1
        current_graph.remove_nodes_from(unique_to_current_graph)
        
        
        ################################################################## step 4:
        # Add nodes to Graph 2 that are in Graph 1 but not in Graph 2
        for node in unique_to_graph_0:
            
            # Find indices where the specified column has the value equal to value_to_match
            index = np.where(nodes_coord_array[:, 0] == node)[0]
            row_missing_node = nodes_coord_array[index,1][0]
            column_missing_node = nodes_coord_array[index,2][0]
               
    
            o_array= np.array( [row_missing_node,column_missing_node] )
            attrs = {node: {'o': o_array}}
            nx.set_node_attributes(current_graph, attrs)
            
            current_graph.add_node( node , **{'o': o_array} )
    
            
                    # The new row to be appended
            new_row = np.array([node, row_missing_node, column_missing_node])
            nodes_coord_array_current = np.append( nodes_coord_array_current, [new_row], axis=0)
        ...
        ################################################################## step 5:
        # Remove edges from Graph 2 that do not exist in Graph 1
        edges_to_remove = [edge for edge in current_graph.edges() if edge not in graph_0.edges() and (edge[1], edge[0]) not in graph_0.edges()]
        current_graph.remove_edges_from(edges_to_remove)
        
               
        ################################################################## step 6:
        # Add only missing edges from Graph 1 to Graph 2
        edges_to_add = [edge for edge in graph_0.edges() if edge not in current_graph.edges() and not np.linalg.norm(current_graph.nodes()[edge[0]]['o'] - current_graph.nodes()[edge[1]]['o']) > 100 and (edge[1], edge[0]) not in current_graph.edges()]
        
        current_graph.add_edges_from(edges_to_add)
        
        nodes_to_remove = [node for node, degree in dict(current_graph.degree()).items() if degree == 0]
        current_graph.remove_nodes_from(nodes_to_remove)
        
        for node in [node for node, degree in dict(graph_0.degree()).items()]:
            node_list = [node for node, degree in dict(current_graph.degree()).items()]
            if node not in node_list:
                print(graph_0.nodes()[node])
                o_array= np.array( [graph_0.nodes()[node]['o'][0],graph_0.nodes()[node]['o'][1]] )
                attrs = {node: {'o': o_array}}
                nx.set_node_attributes(current_graph, attrs)
                current_graph.add_node(node, **{'o': o_array} )
                for node_n in graph_0.neighbors(node): 
                    current_graph.add_edge(node_n, node)      
        ...
        for node in nodes:
            if nodes[node]:
                ...
                nx.set_node_attributes(current_graph, attrs)
                current_graph.add_node(node, **{'o': o_array} )
                node_info_list = [node,graph_0.nodes()[node]['o'][0], graph_0.nodes()[node]['o'][1]]
                nodes_coord_array[counter,:] = np.array(node_info_list)
                if nodes_coord_array[counter, 1] == 0 and nodes_coord_array[counter, 2] == 0:
                    nodes_coord_array[counter, 0] = -1
        ...
        graph_draw_fct (current_horizontal_image, current_graph, frame_number, node_pos_dict,
                        title = "Skeleton graph from frame number " + str(frame_number),
                        node_labels=False,
                        zoom_in = False, nx_direct_edges = False,
                        x_max = 500)
        
        
        # draw graph with nodes and direct edges
        graph_draw_fct (current_horizontal_image , current_graph, frame_number,
                        node_pos_dict,
                        title = "Skeleton graph from frame number " + str(frame_number),
                        node_labels = True,
                        zoom_in = False,
        ...
        # # draw graph with nodes points and white background
        # graph_draw_fct (current_horizontal_image, current_graph, frame_number,
        #                 node_pos_dict,
        #                 title = f"Graph, frame {frame_number}",
        #                 node_labels = False,
        #                 zoom_in = False,
        #                 x_max = 200,
        #                 white_background = True,
        ...
        for i in range(0, max(nodes_2_dict.keys()) , 4):
            ...
            if len(alpha_list_master[frame_number]) <22: 
                ...
                if (i,i+1) in edges_2_dict.keys() or (i+1,i) in edges_2_dict.keys():
                    ...
                    # read the row and column coordinates of the points limiting one flute segment
                    ...
                    #     alpha_list_master[frame_number].append(alpha_list[-1])
                    
                    count_edge += 1
                    
                    if frame_number > 1:
                        if alpha_list_master[frame_number-1][count_edge] < 0 and not alpha_list_master_2[count_edge]:
                            alpha_list_master_2[count_edge] = not alpha_list_master_2[count_edge]
                            count += 1
                            # print(count, alpha_list_master[frame_number-1][count_edge],
                            #       not alpha_list_master_2[count_edge])
                            # print(count_edge, i, i+1)
                    
                else: 
                    
                    count_edge += 1
                    alpha_list_master[frame_number].append(alpha_list_master[frame_number-1][count_edge])
        ...
        if len(alpha_list) != 0:
            ...
            alpha_variance = sum([(val-alpha_average)**2 for val in alpha_list]) / (len(alpha_list)-1)
            alpha_variance_list.append(alpha_variance)
            
            # alpha_average_dict[frame_number] = alpha_average
                ...
                if (i,i+1) in edges_2_dict.keys() or (i+1,i) in edges_2_dict.keys():
                    ...
                    if frame_number > 1:
                        if alpha_list_master[frame_number-1][count_edge] > 0 and not alpha_list_master_2[count_edge]:
                            alpha_list_master_2[count_edge] = not alpha_list_master_2[count_edge]
                            count += 1
                            # print(count, alpha_list_master[frame_number-1][count_edge], not alpha_list_master_2[count_edge])
                            # print(count_edge, i, i+1)
        ...
        alpha_list_vice_master = [alpha_list_master[i] for i in range(len(alpha_list_master)) if len(alpha_list_master[i])>0]
...
if Node_clustering : 
    ...
    for sheet_name, data in excel_data.items():
        if not data.empty:
            ...
            pre_row, pre_col = data.iloc[Ref_frame]['pre_row'], data.iloc[Ref_frame]['pre_col']
            distances = [calculate_distance(pre_row, pre_col, new_row, new_col) for new_row, new_col in zip(data['new_row'], data['new_col'])]
    ...
    for sheet_name, data in excel_data.items():
        if not data.empty:
            ...
            # distances = distances[:compression_end]
            
            color_number = int(sheet_name) % 4
            distances_list.append(distances)
            
            plt.plot(distances, alpha=alpha_transparency, color = f'C{color_number}')   #  label=str(sheet_name)
    ...
    for sheet_name, data in excel_data.items():
        if not data.empty:
            ...
            # Collect distances at positions clustering_Frame_1 and Frame_2 if they exist
            distance_clustering_Frame  = calculate_distance( pre_row, pre_col, data.iloc[clustering_Frame_1]['new_row'], data.iloc[clustering_Frame_1]['new_col']) if len(data) > clustering_Frame_1 else None
            distance_clustering_Framex = calculate_distancex(pre_row, pre_col, data.iloc[clustering_Frame_1]['new_row'], data.iloc[clustering_Frame_1]['new_col']) if len(data) > clustering_Frame_1 else None
            distance_clustering_Framey = calculate_distancey(pre_row, pre_col, data.iloc[clustering_Frame_1]['new_row'], data.iloc[clustering_Frame_1]['new_col']) if len(data) > clustering_Frame_1 else None
            distance_Frame_2  = calculate_distance( pre_row, pre_col, data.iloc[Frame_2]['new_row'], data.iloc[Frame_2]['new_col']) if len(data) > Frame_2 else None
            distance_Frame_2x = calculate_distancex(pre_row, pre_col, data.iloc[Frame_2]['new_row'], data.iloc[Frame_2]['new_col']) if len(data) > Frame_2 else None
            distance_Frame_2y = calculate_distancey(pre_row, pre_col, data.iloc[Frame_2]['new_row'], data.iloc[Frame_2]['new_col']) if len(data) > Frame_2 else None
    
            output_data.append((sheet_name, distance_clustering_Frame, distance_clustering_Framex, distance_clustering_Framey, distance_Frame_2, distance_Frame_2x, distance_Frame_2y))
    ...
    for sheet_name, data in excel_data.items():
        if not data.empty:
            # Ensure pre_row and pre_col are from the first row of the current sheet
            pre_row, pre_col = data.iloc[1]['pre_row'], data.iloc[1]['pre_col']
    ...
    if exch:
        ...
        for node in clustering_graph.nodes():
            if not node in clusters_x[1]:
                node_colors.append('green')  # Color for subgroup 1
            else:
    ...
    # title and show
    plt.title(f'Node groups based on Dendrogram of x-displacement at Frame {clustering_Frame_2}')
    plt.show()
